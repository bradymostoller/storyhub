...
export default function Dashboard() {
  ...
  const handleRemove = async (id: number) => {
    try {
      ...
      if (!response.ok) {
        console.error("Failed to delete fable.");
        return;
      }
  
      setFables(fables.filter((fable) => fable.id !== id));
    } catch (error) {
      console.error("Error deleting fable:", error);
    }
  };
    ...
    <div className="flex min-h-screen flex-col bg-[#1E1E1E] text-white">
      ...
      <header className="flex w-full items-center justify-between bg-[#A260DB] px-8 py-4 text-white">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-3xl font-bold font-header">Story Hub</h1>
        </div>
        <nav>
          <a className="hover:underline font-body">Explore Community Stories</a>
        </nav>
      </header>
